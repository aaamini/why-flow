...
class Config:
    ...
    # DODE settings (pixel space)
    n_train_for_evf = 2000   # number of training samples used as EVF seeds (Y_train)
    n_gen = 2000             # number of generated samples
    dode_steps = 4           # steps for the ODE integrator (e.g., 4 or 8)
    dode_t1 = 1.0            # integrate over t in [0, t1]
    dode_method = "rk2"      # "euler", "rk2", "rk4" depending on evf.Integrator support
...
# =========================
# EVF generator in pixel space (DODE)
# =========================
class EVFPixelGenerator:
    ...
    def dode(self, steps: int, n_samp: int, *, t1: float = 1.0, method: str = "rk2") -> Tensor:
        """
        Deterministic ODE sampling via EVF Integrator in pixel space.
        Starts from standard normal x0 and integrates over a uniform time grid [0, t1].
        Returns generated pixel vectors in [0,1] after clamping.
        """
        field = EmpiricalVectorField(self.Y_train64)
        integ = Integrator(field)
        t_grid = uniform_grid(int(steps), t1=float(t1))
        # Initialize from N(0, I)
        D = self.Y_train64.size(1)
        x0 = torch.randn(int(n_samp), D, device=self.device, dtype=torch.float64)
        xT = integ.integrate(x0, t_grid, method=method, return_traj=False)  # float64
...
def main():
    ...
    # Build EVF pixel generator and sample with DODE
    evf_pix = EVFPixelGenerator(Y_train)
    with torch.no_grad():
        x_gen_flat = evf_pix.dode(
            steps=cfg.dode_steps,
            n_samp=cfg.n_gen,
            t1=cfg.dode_t1,
            method=cfg.dode_method
        ).cpu()  # [n_gen, D], float32 in [0,1]
    print(f"Generated {x_gen_flat.size(0)} pixel samples via DODE (steps={cfg.dode_steps}, method={cfg.dode_method}).")
    ...
    for i in range(n_grid):
        ...
        ax.axis("off")
    grid_path = os.path.join(cfg.out_dir, f"digit{cfg.digit}_gen_grid_pixel_dode_steps{cfg.dode_steps}.png")
    plt.tight_layout()
    ...
    if n_show > 0:
        ...
        for i in range(n_show):
            ...
            ax.set_title("generated (DODE)", fontsize=9)
            ax.axis("off")
        ...
        ex_path = os.path.join(cfg.out_dir, f"digit{cfg.digit}_examples_pixel_dode_steps{cfg.dode_steps}.png")
        plt.tight_layout()
    ...
    plt.hist(nn_gen_to_train.numpy(), bins=cfg.hist_bins, alpha=0.6, density=True,
             label=f"Generated (DODE steps={cfg.dode_steps}) → Train (NN dist)", color="#ff7f0e")
    plt.xlabel("Nearest neighbor distance in pixel space")
    plt.ylabel("Density")
    plt.title(f"Digit {cfg.digit} | NN distance histograms (DODE steps={cfg.dode_steps}, method={cfg.dode_method})")
    plt.legend()
    plt.tight_layout()
    hist_path = os.path.join(cfg.out_dir, f"digit{cfg.digit}_nn_hist_pixel_dode_steps{cfg.dode_steps}.png")
    ...
    if nn_test_to_train is not None:
        ...
        print(f"Median NN dist (pixel, DODE) | Generated: {gen_med:.4f} | Test: {test_med:.4f}")

    print("Done (pixel space EVF with DODE).")
